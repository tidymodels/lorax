...
This function converts an oblique split condition (linear combination)
from a tree-based model into a valid R expression. Oblique splits use a
weighted sum of multiple variables compared to a threshold.
...
- split:
  ...
  - `columns`: character vector - variable names for the linear
    combination.

  - `values`: numeric vector - coefficients for each variable (same
    length as. `columns`)
...
An R expression object that can be evaluated. The expression represents:
`values[1]*columns[1] + .. + values[n]*columns[n] {operator} threshold`.
