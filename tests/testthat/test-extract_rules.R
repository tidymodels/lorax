...
test_that("extract_rules.xgb.Booster validates tree parameter", {
  ...
  bst <- xgboost::xgb.train(
    params = list(max_depth = 2, objective = "binary:logistic", nthread = 1),
    data = dtrain,
    nrounds = 2,
    verbose = 0
  )
...
test_that("extract_rules.xgb.Booster handles single-node trees", {
  ...
  bst <- xgboost::xgb.train(
    params = list(max_depth = 0, objective = "reg:squarederror", nthread = 1),
    data = dtrain,
    nrounds = 1,
    verbose = 0
  )
...
test_that("extract_rules handles regression trees", {
  ...
  bst <- xgboost::xgb.train(
    params = list(max_depth = 3, objective = "reg:squarederror", nthread = 1),
    data = dtrain,
    nrounds = 3,
    verbose = 0
  )
